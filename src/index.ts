...
import { HistoryManager, SessionSummary, formatTs, SessionMessage } from './historyManager.js';
...
program
  ...
  .action(async (sessionId, options, command) => {
    ...
    try {
      ...
      messages.forEach((msg: SessionMessage, idx: number) => {
...
program
  ...
  .action(async (sessionIds: string[], options, command) => {
    const manager = await createManager(command.optsWithGlobals());
    if (!options.yes) {
...
program
  ...
  .action(async (sessionId, textParts: string[] = []) => {
...
async function createManager(options: GlobalOptions): Promise<HistoryManager> {
...
function printSummaryTable(summaries: SessionSummary[]) {
